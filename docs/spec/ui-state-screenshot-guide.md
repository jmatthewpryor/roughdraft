...
| Document | Review handoff after watcher lost | Open a local file while a watcher is connected, then stop the watching process and wait for `/api/review-events/status` to report `watcherCount: 0` | `review-handoff-button` | Button stays mounted once a watcher has been seen for the document (upstream #148); header no longer says `Agent watching`. Before #148 the button unmounted within one 1.5s poll. |
| Document | Review handoff undelivered | Watcher disconnects before handoff, then click the still-visible handoff button | `review-handoff-status` | Popover title: `No agent is watching now`. |
